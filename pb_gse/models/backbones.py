...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, in_channels=3):
        ...
        # Handle different ResNet variants
        if len(num_blocks) > 3:
            self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        else:
            self.layer4 = nn.Identity()  # No layer4 for ResNet32

        # Store feature dimension
        if len(num_blocks) > 3:
            self.feature_dim = 512 * block.expansion  # ResNet18, ResNet34
        else:
            self.feature_dim = 256 * block.expansion  # ResNet32
    ...
    def forward(self, x, return_features=False):
        ...
        # Adaptive pooling to handle different feature map sizes
