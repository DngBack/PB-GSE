...
class GatingNetwork(nn.Module):
    ...
    def __init__(self, input_dim: int, num_models: int, hidden_dims: List[int] = [128, 64],
...
class FeatureExtractor:
    ...
    def extract(self, model_probs: List[torch.Tensor], 
                group_onehot: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.config.get('use_disagreement', True):
            if len(model_probs) > 1:
                ...
                disagreement_scalar = torch.mean(disagreement, dim=1, keepdim=True)  # [batch, 1]
...
class PACBayesGating(nn.Module):
    ...
    def __init__(self, input_dim: int, num_models: int, config: Dict):
        ...
        if self.config['method'] == 'gaussian':
            ...
            self.posterior = GaussianPosterior(
                total_params, 
                self.config['posterior_std_init']
            )
    ...
    def _reconstruct_network_params(self, flat_params: torch.Tensor) -> List[Tuple[torch.Tensor, torch.Tensor]]:
        ...
        for in_dim, out_dim in self.layer_shapes:
            ...
            weight = flat_params[start_idx:start_idx + weight_size].view(out_dim, in_dim)
    ...
    def _forward_with_params(self, x: torch.Tensor, params: List[Tuple[torch.Tensor, torch.Tensor]]) -> torch.Tensor:
    ...
    def compute_ensemble_probs(self, model_probs: List[torch.Tensor], 
    ...
    def pac_bayes_bound(self, empirical_loss: torch.Tensor, n_samples: int, 
...
class BalancedLinearLoss:
    ...
    def __call__(self, ensemble_probs: torch.Tensor, targets: torch.Tensor, 
        ...
        for i in range(batch_size):
            ...
            for class_id in range(len(probs)):
                ...
                rhs += (1/self.alpha[class_group] - self.mu[class_group]) * probs[class_id]
...
def create_gating_features(model_probs_list: List[List[torch.Tensor]], 
