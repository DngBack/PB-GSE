...
class PluginRule:
    ...
    def __init__(self, alpha: Dict[int, float], mu: Dict[int, float], 
                 rejection_cost: float, group_info: Dict):
        self.alpha = alpha
        self.mu = mu
...
class FixedPointSolver:
    ...
    def solve_alpha(self, probs: torch.Tensor, targets: torch.Tensor, 
                   group_ids: torch.Tensor, mu: Dict[int, float], 
                   rejection_cost: float, group_info: Dict, 
                   num_groups: int = 2) -> Dict[int, float]:
        ...
        for iteration in range(self.max_iterations):
            ...
            for group_id in range(num_groups):
                ...
                if group_mask.sum() > 0:
                    accept_rate = group_accept_mask.sum().float() / group_mask.sum().float()
            ...
            logging.warning(f"Fixed-point did not converge after {self.max_iterations} iterations")
...
class GridSearchOptimizer:
    ...
    def optimize(self, probs: torch.Tensor, targets: torch.Tensor, 
                group_ids: torch.Tensor, rejection_cost: float, 
                group_info: Dict, metric: str = 'balanced_selective_error') -> Tuple[Dict[int, float], Dict[int, float], float]:
        ...
        for lambda_val in self.lambda_grid:
            ...
            score = self._evaluate_performance(
                probs, targets, group_ids, alpha, mu, rejection_cost, 
                group_info, metric
            )
        ...
        logging.info(f"Best λ: {best_lambda}, Best score: {best_score}")
        return best_alpha, best_mu, best_lambda
    
    def _evaluate_performance(self, probs: torch.Tensor, targets: torch.Tensor,
                            group_ids: torch.Tensor, alpha: Dict[int, float], 
                            mu: Dict[int, float], rejection_cost: float,
                            group_info: Dict, metric: str) -> float:
    ...
    def _compute_balanced_selective_error(self, predictions: torch.Tensor, 
                                        targets: torch.Tensor, rejections: torch.Tensor,
                                        group_ids: torch.Tensor) -> float:
    ...
    def _compute_worst_group_error(self, predictions: torch.Tensor, 
                                 targets: torch.Tensor, rejections: torch.Tensor,
                                 group_ids: torch.Tensor) -> float:
...
class PluginOptimizer:
    ...
    def optimize(self, probs: torch.Tensor, targets: torch.Tensor,
                group_ids: torch.Tensor, group_info: Dict) -> Tuple[Dict[int, float], Dict[int, float]]:
        ...
        logging.info(f"α: {best_alpha}")
        logging.info(f"μ: {best_mu}")
        logging.info(f"λ: {best_lambda}")
        
        return best_alpha, best_mu
    
    def create_plugin_rule(self, alpha: Dict[int, float], mu: Dict[int, float],
                          group_info: Dict) -> PluginRule:
        """Create plugin rule with optimized parameters"""
        return PluginRule(
            alpha, mu, self.config['rejection_cost'], group_info
        )
...
def evaluate_plugin_rule(plugin_rule: PluginRule, probs: torch.Tensor, 
                        targets: torch.Tensor, group_ids: torch.Tensor) -> Dict[str, float]:
