...
class GroupTemperatureScaling(nn.Module):
    ...
    def calibrate(
        ...
        for group_id in range(self.num_groups):
            ...
            temp_param = nn.Parameter(
                torch.ones(1).to(group_logits.device)
                * self.temperatures[group_id].item()
            )
            ...
            with torch.no_grad():
                ...
                self.temperatures.data[group_id] = temp_param.item()
...
class GroupVectorScaling(nn.Module):
    ...
    def calibrate(
        ...
        for group_id in range(self.num_groups):
            ...
            W_param = nn.Parameter(self.W[group_id].clone().to(group_logits.device))
            b_param = nn.Parameter(self.b[group_id].clone().to(group_logits.device))
...
def group_expected_calibration_error(
    probs: np.ndarray, labels: np.ndarray, group_ids: np.ndarray, n_bins: int = 15
) -> Dict[int, float]:
    ...
    for group_id in unique_groups:
        ...
        if mask.sum() > 0:
            ...
            group_labels = labels[mask]
            group_eces[group_id] = expected_calibration_error(
                group_probs, group_labels, n_bins
            )
        else:
...
class ModelCalibrator:
    ...
    def calibrate(self, dataloader, group_info: Dict, device: str = "cuda"):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(dataloader):
                ...
                group_ids = torch.tensor(
                    [group_info["class_to_group"][target.item()] for target in targets]
                ).to(device)
    ...
    def get_calibrated_probs(
        self, inputs: torch.Tensor, group_ids: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
