...
def load_model_probs(probs_dir: str, model_names: List[str], split: str = 'train') -> Tuple[List[torch.Tensor], torch.Tensor]:
...
def create_gating_dataset(model_probs_list: List[torch.Tensor], targets: torch.Tensor,
                         group_info: Dict, config: Dict) -> TensorDataset:
    ...
    group_onehot = torch.nn.functional.one_hot(group_ids, num_classes=num_groups).float()
...
def train_gating_epoch(gating_model, dataloader, optimizer, alpha, mu, rejection_cost,
                      group_info, config, device, epoch=0):
    ...
    for batch_idx, (features, targets, group_ids) in enumerate(dataloader):
        ...
        if config['gating']['pac_bayes']['method'] == 'gaussian':
            ...
            for _ in range(num_samples):
                ...
                dummy_probs = [torch.softmax(torch.randn(batch_size, num_classes), dim=1).to(device)
                              for _ in range(gating_model.num_models)]
                
                # Compute ensemble probabilities
                ensemble_probs = gating_model.compute_ensemble_probs(dummy_probs, gating_weights)
...
def validate_gating(gating_model, dataloader, alpha, mu, rejection_cost,
                   group_info, device):
...
def main():
    ...
    parser.add_argument('--probs_dir', type=str, required=True, help='Directory with model probabilities')
    ...
    train_dataset, input_dim = create_gating_dataset(train_probs, train_targets, group_info, config)
    ...
    with torch.no_grad():
        ...
        dummy_probs = [torch.softmax(torch.randn(cal_features.size(0), 10), dim=1).to(device)
                      for _ in range(num_models)]
        ensemble_probs = gating_model.compute_ensemble_probs(dummy_probs, gating_weights)
    
    alpha, mu = plugin_optimizer.optimize(ensemble_probs, cal_targets, cal_group_ids, group_info)
    ...
    for epoch in range(epochs):
        ...
        train_loss, train_bound = train_gating_epoch(
            gating_model, train_loader, optimizer, alpha, mu, 
            rejection_cost, group_info, config, device, epoch
        )
