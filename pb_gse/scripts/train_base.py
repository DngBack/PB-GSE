...
def get_class_frequencies(dataset, group_info):
    ...
    if class_counts:
        ...
        frequencies = [1] * 10  # Default for CIFAR-10
...
def save_model_probs(model, dataloader, save_path, device):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            ...
            start_idx = batch_idx * len(
                inputs
            )  # More accurate than dataloader.batch_size
    ...
    # Save to file
    torch.save(
        {"probs": all_probs, "targets": all_targets, "indices": all_indices}, save_path
    )
...
def main():
    ...
    # Create data loaders
    # Handle different dataset types for getting targets
    if hasattr(train_dataset, "indices"):  # Subset dataset
        train_targets = [train_dataset.dataset[idx][1] for idx in train_dataset.indices]
    else:
        train_targets = [train_dataset[i][1] for i in range(len(train_dataset))]

    # Convert tensors to integers if needed
    train_targets = [
        t.item() if isinstance(t, torch.Tensor) else t for t in train_targets
    ]

    train_sampler = get_sampler(
        config, train_dataset, train_targets, group_info, "train"
    )
    ...
    save_model_probs(
        model, test_loader, os.path.join(prob_save_dir, "test.pth"), device
    )
