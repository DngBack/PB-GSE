...
def run_inference_and_evaluation(config: dict, args):
    ...
    logging.info(
        f"Balanced Selective Error: {final_metrics['balanced_selective_error']:.3f}"
    )
    logging.info(
        f"Worst-Group Selective Error: {final_metrics['worst_group_selective_error']:.3f}"
    )
...
def main():
    ...
    parser.add_argument(
        "--skip_training", action="store_true", help="Skip training stages"
    )
    parser.add_argument(
        "--pbgse_only",
        action="store_true",
        help="Run only PB-GSE method with synthetic data",
    )
    parser.add_argument(
        "--use_existing_probs",
        type=str,
        help="Use existing model probabilities directory",
    )
    ...
    if args.pbgse_only:
        # Run only PB-GSE method with synthetic data
        logging.info("=== Running PB-GSE Method Only (with synthetic data) ===")

        cmd = [
            "python",
            "pb_gse/scripts/run_pbgse_only.py",
            "--config",
            args.config,
            "--save_dir",
            args.save_dir,
    ...
    if not args.skip_training:
        ...
        if config["experiment"]["stages"]["train_base_models"]:
            ...
            for model_config in model_configs:
                ...
                if not run_stage(
                    f"Train {model_config}", "pb_gse/scripts/train_base.py", stage_args
                ):
                    success = False
                    break
        ...
        if success and config["experiment"]["stages"]["train_gating"]:
            ...
            stage_args = [
                ...
                args.config,
                "--probs_dir",
                os.path.join(args.save_dir, "probs_calibrated"),
                "--save_dir",
                args.save_dir,
                "--device",
                args.device,
            ...
            if not run_stage(
                "Gating Training", "pb_gse/scripts/train_gating_pacbayes.py", stage_args
            ):
                success = False
